function [ assessment ] = getAssessment( gt_dir, res_dir )
...
res_files = ListFiles(res_dir);

range = size(res_files,1);


for i=1:range,
    % Read files
    gt_frame = imread(strcat(gt_dir,'/','gt',res_files(i).name(...
        3:size(res_files(i).name,2)-3), 'png'));
    res_frame = logical(imread(strcat(res_dir,'/',res_files(i).name)))*255;
