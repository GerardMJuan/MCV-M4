...
for k=1:size(ListNames,2); % For all datasets which where asked
    ...
    dir = strcat('datasets/',name,'/input');          % Location of the original images
    res_dir = strcat('results/',name,'-gauss');      % Location of the results
    gt_dir = strcat('datasets/',name,'/groundtruth'); % Location of the groundtruth
